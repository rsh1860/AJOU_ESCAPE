...
public class DA_Q : MonoBehaviour {
    Scene m_Scene;//added
	...
	void Start () {
		...
		String sqlQuary = "SELECT * FROM PROBLEM WHERE TYPE_ID = 'K1' ORDER BY random()";
	...
	IEnumerator submit()
	{
		...
		if (useranswer == answer) {
            ...
            if (sceneName == "DA")
            {
                ...
                SceneManager.LoadScene("DA 1");
            }

            else if (sceneName == "DA 1")
            {
                GameObject.Find("Canvas").transform.Find("Panel (1)").gameObject.SetActive(true);
                yield return new WaitForSecondsRealtime(1);
                SceneManager.LoadScene("DA 2");
            }

            else if (sceneName == "DA 2")
            {
                GameObject.Find("Canvas").transform.Find("Panel (1)").gameObject.SetActive(true);
                yield return new WaitForSecondsRealtime(1);
                SceneManager.LoadScene("DA 3");
            }

            else if (sceneName == "DA 3")
            {
                GameObject.Find("Canvas").transform.Find("Panel (1)").gameObject.SetActive(true);
                yield return new WaitForSecondsRealtime(1);
